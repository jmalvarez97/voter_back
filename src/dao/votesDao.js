...
async function findRates() {
  ...
  let ft = await (
    await Votes.count({ attributes: ["firstTeam"], group: "firstTeam" })
  ).map((o) => (suma[o.firstTeam] = o.count));

  let lt = await (
    await Votes.count({ attributes: ["lastTeam"], group: "lastTeam" })
  ).map((o) => {
    if (suma[o.lastTeam]) suma[o.lastTeam] += o.count;
    else suma[o.lastTeam] = o.count;
  });
  
  let wins = await (
    await Votes.count({ attributes: ["winner"], group: "winner" })
  ).map((o) => {
